...
function calculateFinalPredictionImproved(
  ...
  const goodBook = Math.abs(bookScore) > 0.2; // relajado para más señales válidas
  ...
  const weakFlow = Math.abs(flowScore) < 0.08; // más permisivo

  const prelimExpectedMove = Math.abs(finalScore) * 1.5;
  const isLikelySideways =
    Math.abs(finalScore) < 0.18 ||
    prelimExpectedMove < 0.15 ||
  ...
  const flowVeryStrong = Math.abs(flowScore) > 0.35; // relajado
  ...
  const priceVwapOk = extras
    ? Math.abs(extras.priceVsVwapPct) >= 0.0007 || Math.abs(flowScore) >= 0.12
    : true;
  const vwapSlopeOk = extras
    ? Math.abs(extras.vwapSlope) >= 0.0003 || Math.abs(flowScore) >= 0.4
    : true;
  const recentBookStrong = extras
    ? (Math.abs(extras.recentAvgImbalance) >= 0.08 &&
        Math.abs(extras.recentDepthAsymmetry) >= 0.06) ||
      extras.recentSpreadTightening > 0.15
    : true;
  ...
  if (
    ...
    direction = 'SIDEWAYS';
  } else if (finalScore > 0.22) {
    // umbral decisión relajado
    direction = 'UP';
  } else if (finalScore < -0.22) {
    direction = 'DOWN';
  } else {
